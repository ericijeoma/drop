...
export default function TrackDeliveryScreen() {
  ...
  const { isLoading } = useQuery({
    ...
    enabled:  !!orderId,
    onSuccess: (data) => { if (data) setOrder(data); },
  });
  ...
  const { isRealtime } = useRealtimeWithFallback<Record<string, unknown>>({
    table:         'orders',
    event:         'UPDATE',
    filter:        orderId ? { column: 'id', value: orderId } : undefined,
    onData:        () => fallbackQuery(),   // refresh full entity on any update
    fallbackQuery,
    pollIntervalMs: 8_000,  // 8s on 3G — more frequent than rides (delivery status changes faster)
  });
  ...
  const statusConfig: Record<string, { label: string; color: string; icon: string }> = {
    ...
    assigned:   { label: 'Driver heading to pickup',  color: theme.info,    icon: '🏍' },
  ...
  const status = statusConfig[order.status] ?? {
    label: order.status, color: theme.textSecondary, icon: '📦',
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      >
              ...
              return (
                <View key={step.key} style={styles.progressStep}>
                  ...
                  {index < steps.length - 1 && (
                    <View style={[
                      styles.progressLine,
                      { backgroundColor: index < currentStepIndex ? theme.brand : theme.border },
                    ]} />
                  )}
                </View>
              );
        ...
        {/* Package info */}
        <View style={[styles.packageInfo, { backgroundColor: theme.surfaceElevated, borderColor: theme.border }]}>
          ...
          <Text style={[styles.packageSize, { color: theme.textTertiary }]}>
            {order.packageSize.charAt(0).toUpperCase() + order.packageSize.slice(1)} package
          </Text>
        </View>
        ...
        {order.deliveryPhotoUrl && (
          ...
          >
            ...
            <Text style={[styles.photoUrl, { color: theme.info }]} numberOfLines={1}>
              Photo received ✓
            </Text>
          </View>
        )}
        ...
        <View style={styles.fareRow}>
          <Text style={[styles.fareLabel, { color: theme.textSecondary }]}>Delivery fee</Text>
          <Text
        ...
        {order.status === 'delivered' && order.paymentStatus === 'pending' && !isComplete && (
          <PayNowButton
...
const styles = StyleSheet.create({
  ...
  sheet:              { maxHeight: '55%', borderTopLeftRadius: 24, borderTopRightRadius: 24, borderTopWidth: 1 },
  sheetContent:       { padding: 20, gap: 14 },
  statusRow:          { flexDirection: 'row', alignItems: 'center', gap: 10 },
  statusIcon:         { fontSize: 20 },
  statusText:         { fontSize: 16, fontWeight: '600', flex: 1 },
  progressContainer:  { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' },
  progressStep:       { alignItems: 'center', flex: 1, position: 'relative' },
  progressDot:        { width: 12, height: 12, borderRadius: 6, marginBottom: 4 },
  progressLine:       { position: 'absolute', top: 5, left: '50%', right: '-50%', height: 2 },
