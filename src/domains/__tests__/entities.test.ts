...
describe('User entity', () => {
  ...
  it('rejects invalid phone number — no country code', () => {
    expect(() => User.create({ ...validUserProps, phone: '08012345678' }))
      .toThrow(DomainError);
  });

  it('rejects invalid phone number — letters', () => {
    expect(() => User.create({ ...validUserProps, phone: '+234abcdefg' }))
      .toThrow(DomainError);
  });

  it('rejects invalid phone number — too short', () => {
    expect(() => User.create({ ...validUserProps, phone: '+2341234' }))
      .toThrow(DomainError);
  });
...
describe('Ride entity', () => {
  ...
  it('rejects zero fare', () => {
    expect(() => Ride.create({ ...validRideProps, fareAmount: 0 }))
      .toThrow(DomainError);
  });

  it('rejects negative fare', () => {
    expect(() => Ride.create({ ...validRideProps, fareAmount: -100 }))
      .toThrow(DomainError);
  });

  it('rejects zero distance', () => {
    expect(() => Ride.create({ ...validRideProps, distanceKm: 0 }))
      .toThrow(DomainError);
  });

  it('rejects same pickup and dropoff', () => {
    const sameCoords = { lat: 6.4550, lng: 3.3841 };
    expect(() => Ride.create({
      ...validRideProps,
  ...
  it('rejects distance > 500km', () => {
    expect(() => Ride.create({ ...validRideProps, distanceKm: 501 }))
      .toThrow(DomainError);
  });
  ...
  describe('State transitions', () => {
    ...
    it('completed → active is ILLEGAL', () => {
      ...
      expect(() => ride.transition('active')).toThrow('INVALID_RIDE_TRANSITION');
    });
...
describe('RidePolicy', () => {
  ...
  it('minimum fare for van > car > motorbike', () => {
    ...
    expect(RidePolicy.MIN_FARE.car).toBeGreaterThan(RidePolicy.MIN_FARE.motorbike);
  });
...
describe('DriverProfile entity', () => {
  ...
  it('rejects creating verified profile for unverified driver', () => {
    expect(() => DriverProfile.create({ ...validDriverProps, isVerified: false }))
      .toThrow(DomainError);
  });

  it('createUnverified succeeds for registration flow', () => {
    const driver = DriverProfile.createUnverified({ ...validDriverProps, isVerified: false });
  ...
  it('updateLocation throws when offline', () => {
    const driver = DriverProfile.create(validDriverProps);
    expect(() => driver.updateLocation({ lat: 6.5244, lng: 3.3792 }))
      .toThrow(DomainError);
  });
});
...
describe('Order entity', () => {
  ...
  it('rejects empty package description', () => {
    expect(() => Order.create({ ...validOrderProps, packageDescription: '   ' }))
      .toThrow(DomainError);
  });
  ...
  it('cannot confirm delivery on pending order', () => {
    const order = Order.create(validOrderProps);
    expect(() => order.confirmDelivery('https://example.com/photo.jpg'))
      .toThrow(DomainError);
  });
});
...
describe('Payment entity', () => {
  ...
  it('rejects zero amount', () => {
    expect(() => Payment.create({ ...validPaymentProps, amount: 0 }))
      .toThrow(DomainError);
  });
  ...
  it('rejects missing idempotency key', () => {
    expect(() => Payment.create({ ...validPaymentProps, idempotencyKey: '' }))
      .toThrow(DomainError);
  });
