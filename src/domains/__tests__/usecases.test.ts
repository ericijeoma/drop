...
function makeDriver(overrides = {}): DriverProfile {
  return DriverProfile.create({
    ...
    status: "online",
    currentLocation: { lat: 6.45, lng: 3.38 },
    rating: 4.8,
    totalTrips: 120,
    isVerified: true,
    fcmToken: null,
...
function mockRideRepo(overrides = {}) {
  return {
    ...
    cancel: jest
      .fn()
      .mockResolvedValue(makePendingRide({ status: "cancelled" })),
...
function mockOrderRepo(overrides = {}) {
  return {
    ...
    getHistoryForCustomer: jest
      .fn()
      .mockResolvedValue({ data: [], total: 0, hasMore: false }),
    acceptOrderAtomic: jest
      .fn()
      .mockResolvedValue(
        makeOrder({ status: "assigned", driverId: "driver-1" }),
      ),
    confirmDelivery: jest
      .fn()
...
function mockActivityService(overrides = {}) {
  return {
    assertNoActiveActivity: jest.fn().mockResolvedValue(undefined),
    ...overrides,
  } as unknown as CustomerActivityService; // ← add this cast
}
...
jest.mock("@/shared/lib/supabase", () => ({
  supabase: {
    from: () => ({
      ...
      select: jest
        .fn()
        .mockResolvedValue({ data: [{ score: 5 }], error: null }),
    }),
    auth: {
      getUser: jest
        .fn()
...
describe("BookRideUseCase", () => {
  ...
  it("throws if customer is banned", async () => {
    const authRepo = mockAuthRepo({
      getUserById: jest
        .fn()
        .mockResolvedValue(makeCustomer({ isBanned: true })),
    });
  ...
  it("throws if user is a driver, not a customer", async () => {
    const authRepo = mockAuthRepo({
      getUserById: jest
        .fn()
        .mockResolvedValue(makeCustomer({ role: "driver" })),
    });
    const useCase = new BookRideUseCase(mockRideRepo(), authRepo);
...
describe("AcceptRideUseCase", () => {
  it("accepts a ride successfully", async () => {
    ...
    await expect(useCase.execute("ride-1", "user-2")).resolves.toEqual({
      rideId: "ride-1",
    });
  });
  ...
  it("throws if driver not found", async () => {
    ...
    await expect(useCase.execute("ride-1", "unknown-user")).rejects.toThrow(
      "DRIVER_NOT_FOUND",
    );
  });
  ...
  it("throws if driver is offline", async () => {
    ...
    const driverRepo = mockDriverRepo({
      getByUserId: jest.fn().mockResolvedValue(offlineDriver),
    });
    const useCase = new AcceptRideUseCase(mockRideRepo(), driverRepo);
    await expect(useCase.execute("ride-1", "user-2")).rejects.toThrow(
  ...
  it("throws RIDE_NOT_AVAILABLE when another driver already accepted", async () => {
    const rideRepo = mockRideRepo({
      acceptRideAtomic: jest
        .fn()
        .mockRejectedValue(new Error("RIDE_NOT_AVAILABLE")),
    });
...
describe("CompleteRideUseCase", () => {
  it("completes a ride and frees the driver", async () => {
    const rideRepo = mockRideRepo({
      getById: jest.fn().mockResolvedValue(makeActiveRide()),
    });
    const driverRepo = mockDriverRepo();
  ...
  it("throws if driver does not own the ride", async () => {
    ...
    const rideRepo = mockRideRepo({
      getById: jest.fn().mockResolvedValue(ride),
    });
    const useCase = new CompleteRideUseCase(
      rideRepo,
      mockDriverRepo(),
      mockPaymentRepo(),
    );
...
describe("PlaceOrderUseCase", () => {
  ...
  it("throws if customer has active ride", async () => {
    const activityService = mockActivityService({
      assertNoActiveActivity: jest
        .fn()
        .mockRejectedValue(
          new DomainError(
  ...
  it("throws if package description is empty", async () => {
    ...
    await expect(
      useCase.execute({ ...validInput, packageDescription: "   " }),
...
describe("ConfirmDeliveryUseCase", () => {
  it("confirms delivery with photo", async () => {
    ...
    const useCase = new ConfirmDeliveryUseCase(
      orderRepo,
      mockDriverRepo(),
      mockPaymentRepo(),
    );
    await expect(
      useCase.execute("order-1", "user-2", "https://cdn.drop.com/photo.jpg"),
  ...
  it("throws if photo URL is empty", async () => {
    ...
    await expect(useCase.execute("order-1", "user-2", "")).rejects.toThrow(
      "MISSING_PHOTO",
    );
  });
...
describe("UpdateDriverStatusUseCase", () => {
  ...
  it("throws if driver not found", async () => {
    ...
    const useCase = new UpdateDriverStatusUseCase(driverRepo);
    await expect(useCase.execute("unknown", "online")).rejects.toThrow(
      "DRIVER_NOT_FOUND",
    );
  });
  ...
  it("throws if going offline while busy", async () => {
    ...
    const driverRepo = mockDriverRepo({
      getByUserId: jest.fn().mockResolvedValue(busyDriver),
    });
    const useCase = new UpdateDriverStatusUseCase(driverRepo);
    await expect(useCase.execute("user-2", "offline")).rejects.toThrow(
      "DRIVER_BUSY",
    );
  });
});
...
describe("LoginUseCase", () => {
  ...
  it("sendOtp throws with invalid phone", async () => {
    ...
    await expect(useCase.sendOtp("08012345678")).rejects.toThrow(
      "INVALID_PHONE",
    );
  });
  ...
  it("verifyOtp throws if token is not 6 digits", async () => {
    const useCase = new LoginUseCase(mockAuthRepo());
    await expect(useCase.verifyOtp("+2348012345678", "123")).rejects.toThrow(
      "INVALID_OTP",
    );
  });
  ...
  it("verifyOtp throws if returned user is banned", async () => {
    ...
    const useCase = new LoginUseCase(authRepo);
    await expect(useCase.verifyOtp("+2348012345678", "123456")).rejects.toThrow(
      "USER_BANNED",
    );
  });
});
