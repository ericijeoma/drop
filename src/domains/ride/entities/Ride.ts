...
import type { Coords, RideStatus, VehicleType, PaymentStatus } from '@/shared/types';
...
export class Ride {
  ...
  static create(props: RideProps): Ride {
    ...
    Ride.validatePickupDropoffDifferent(props.pickupCoords, props.dropoffCoords);
  ...
  get vehicleType(): VehicleType { return this.props.vehicleType; }
  ...
  transition(newStatus: RideStatus): void {
    ...
    if (!allowed.includes(newStatus)) {
      throw new DomainError(
        `Cannot transition ride from '${this._status}' to '${newStatus}'.`,
  ...
  assignDriver(driverId: string): void {
    ...
    if (this._driverId !== null) {
      throw new DomainError('Ride already has a driver assigned.', 'DRIVER_ALREADY_ASSIGNED');
    }
  ...
  isTerminal(): boolean   { return ['completed', 'cancelled', 'timed_out'].includes(this._status); }
  ...
  private static validateDistance(distanceKm: number): void {
    if (distanceKm <= 0) {
      throw new DomainError('Distance must be greater than zero.', 'INVALID_DISTANCE');
    }
    if (distanceKm > 500) {
      throw new DomainError('Distance exceeds maximum allowed (500km).', 'DISTANCE_TOO_LARGE');
    }
  }

  private static validateCoords(coords: Coords, label: string): void {
    if (coords.lat < -90 || coords.lat > 90) {
      throw new DomainError(`Invalid ${label} latitude: ${coords.lat}`, 'INVALID_COORDS');
    }
    if (coords.lng < -180 || coords.lng > 180) {
      throw new DomainError(`Invalid ${label} longitude: ${coords.lng}`, 'INVALID_COORDS');
    }
  }

  private static validatePickupDropoffDifferent(pickup: Coords, dropoff: Coords): void {
