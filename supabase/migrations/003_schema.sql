-- ============================================================
-- Drop — Complete Database Schema (v4)
...
--   Tables / Enums → DO $$ EXCEPTION WHEN duplicate ...
--   Functions      → CREATE OR REPLACE
--   Indexes        → CREATE INDEX IF NOT EXISTS
--   Policies       → DROP POLICY IF EXISTS → CREATE POLICY
--                    DROP list includes BOTH old names (from
--                    previous versions) AND new names (from
--                    this script) so re-runs never hit a
--                    duplicate error.
--
-- All fixes applied:
--   ✓ search_path pinned on all 7 functions
--   ✓ SECURITY DEFINER functions: search_path = '' +
--     fully-qualified names (prevents privilege escalation)
--   ✓ All RLS auth calls wrapped in (SELECT ...) for
--     initPlan caching (evaluated once per query, not per row)
--   ✓ TO authenticated on all policies (anon role never
--     triggers policy evaluation)
--   ✓ No FOR ALL policies — every action declared explicitly
--   ✓ Exactly one policy per role + action per table
--     (admin access folded in via OR, not a separate policy)
--   ✓ rides / orders: SELECT policies merged into one
--   ✓ users UPDATE: merged into one with role-escalation guard
--   ✓ drivers UPDATE: merged into one
--   ✓ notifications UPDATE: WITH CHECK added
--   ✓ 4 FK indexes added to ratings
--   ✓ Duplicate idx_users_phone removed
--   ✓ public.instruments dropped
--
-- Pending manual action (cannot be resolved via SQL):
--   • spatial_ref_sys RLS + Extension in Public warnings:
--     PostGIS is owned by supabase_admin — only Supabase
--     Support can move it to the extensions schema.
--     Open a ticket at supabase.com/support.
-- ============================================================
...
-- ── Hygiene: drop orphaned table ──────────────────────────────
DROP TABLE IF EXISTS public.instruments;


-- ── Indexes ───────────────────────────────────────────────────
-- idx_users_phone intentionally absent — the UNIQUE constraint
-- on users.phone already creates an implicit index.
-- Drop it if it still exists from an older run.
...
CREATE INDEX IF NOT EXISTS idx_rides_timeout
  ON rides(timeout_at) WHERE status = 'pending';
...
-- ratings — all four FK columns covered
-- ride_id / order_id: partial indexes since only one is ever
-- populated per row (the other is always NULL).
...
CREATE INDEX IF NOT EXISTS idx_ratings_ride_id
  ON ratings(ride_id)  WHERE ride_id  IS NOT NULL;
...
-- ── Policies ─────────────────────────────────────────────────
--
-- Idempotency rule: every DROP block lists ALL names this table's
-- policies have EVER had across every version of this schema.
-- That guarantees DROP POLICY IF EXISTS clears the slate on both
-- a fresh database (all no-ops) and an existing one (removes
-- whatever version is currently installed), so CREATE POLICY
-- always succeeds on the first attempt regardless of history.


-- ── Policies — users ──────────────────────────────────────────

-- Drop every name this table's policies have ever had
...
-- Current names (also dropped so re-runs are clean)
...
-- Single UPDATE policy covering both regular users and admins.
-- WITH CHECK distinguishes the two paths:
--   Admin  → no restrictions
--   User   → cannot change their own role (self-escalation guard)
...
DROP POLICY IF EXISTS "authenticated: read driver profiles" ON drivers;
DROP POLICY IF EXISTS "admin: full access to drivers"       ON drivers;
...
-- Single UPDATE policy: driver updates their own row OR admin
CREATE POLICY "drivers: update"
...
CREATE POLICY "drivers: insert"
  ON drivers FOR INSERT
  TO authenticated
  WITH CHECK ((SELECT is_admin()));

CREATE POLICY "drivers: delete"
...
DROP POLICY IF EXISTS "customers: see own rides"               ON rides;
DROP POLICY IF EXISTS "drivers: see assigned or pending rides" ON rides;
DROP POLICY IF EXISTS "customers: insert rides"                ON rides;
...
-- Single SELECT covering customers, drivers, and admins
CREATE POLICY "rides: select"
  ON rides FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "rides: insert"
  ON rides FOR INSERT
  TO authenticated
  WITH CHECK (customer_id = (SELECT current_user_id()));

CREATE POLICY "rides: update"
  ON rides FOR UPDATE
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "drivers: see assigned or pending orders" ON orders;
...
DROP POLICY IF EXISTS "users: see ratings"            ON ratings;
DROP POLICY IF EXISTS "customers: insert own ratings" ON ratings;
-- Current names
DROP POLICY IF EXISTS "ratings: select"               ON ratings;
DROP POLICY IF EXISTS "ratings: insert"               ON ratings;

CREATE POLICY "ratings: select"
...
DROP POLICY IF EXISTS "users: insert own logs" ON app_logs;
DROP POLICY IF EXISTS "admin: read all logs"   ON app_logs;
-- Current names
DROP POLICY IF EXISTS "app_logs: select"       ON app_logs;
DROP POLICY IF EXISTS "app_logs: insert"       ON app_logs;

CREATE POLICY "app_logs: select"
  ON app_logs FOR SELECT
  TO authenticated
  USING ((SELECT is_admin()));

CREATE POLICY "app_logs: insert"
...
DROP POLICY IF EXISTS "admin: full access to audit log"  ON admin_audit_log;
...
DROP POLICY IF EXISTS "admin: update audit log"          ON admin_audit_log;
DROP POLICY IF EXISTS "admin: delete audit log"          ON admin_audit_log;
-- Current names
DROP POLICY IF EXISTS "admin_audit_log: select"          ON admin_audit_log;
DROP POLICY IF EXISTS "admin_audit_log: insert"          ON admin_audit_log;
DROP POLICY IF EXISTS "admin_audit_log: update"          ON admin_audit_log;
DROP POLICY IF EXISTS "admin_audit_log: delete"          ON admin_audit_log;

CREATE POLICY "admin_audit_log: select"
  ON admin_audit_log FOR SELECT
  TO authenticated
  USING ((SELECT is_admin()));

CREATE POLICY "admin_audit_log: insert"
  ON admin_audit_log FOR INSERT
  TO authenticated
  WITH CHECK ((SELECT is_admin()));

CREATE POLICY "admin_audit_log: update"
  ON admin_audit_log FOR UPDATE
  TO authenticated
  USING ((SELECT is_admin()))
  WITH CHECK ((SELECT is_admin()));

CREATE POLICY "admin_audit_log: delete"
...
BEGIN
  RAISE NOTICE '══════════════════════════════════════════════════════════';
  RAISE NOTICE '✓ Drop schema v4 installed successfully';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '';
  RAISE NOTICE '  RLS: enabled — 1 policy per role+action per table';
  RAISE NOTICE '  Realtime: rides, orders, drivers, notifications';
  RAISE NOTICE '';
  RAISE NOTICE '  Pending manual action:';
  RAISE NOTICE '    • spatial_ref_sys + Extension in Public:';
  RAISE NOTICE '      Open a Supabase Support ticket to move';
  RAISE NOTICE '      PostGIS to the extensions schema.';
  RAISE NOTICE '══════════════════════════════════════════════════════════';
